///////////////////////////////////////////////////////////////////
//           ILI8341 Color LCD DEMO with STM32F411 MCU           //
//               **** PARALLEL DISPLAY! ****                     //
//           8 bit parallel mode, 4 CTRLlines                    //
///////////////////////////////////////////////////////////////////
//  Mikrocontroller:  STM32F411                                  //
//                                                               //
...
//Display buffer
char *oldbuf = (char*)"         ";
int bcolor = DARKBLUE; //Standard Background Color
...
void lcd_send(int, int);                           //Write a byte of data via parallel bus
...
{	
	LCDGPIO->ODR |= (1 << LCD_RD); //RD high to write data	
	LCDGPIO->ODR |= (1 << LCD_WR);
	
	if(!dc) //Cmd (0) or Data(1)?
	{
	    LCDGPIO->ODR &= ~(1 << LCD_RS);  //Cmd=0
	}
	else
	{
	    LCDGPIO->ODR |= (1 << LCD_RS);     //Data=1
	}
...
int main(void)
{
    ...
    for(t0 = 0; t0 < 12; t0++)
    { 
        LCDGPIO->MODER  |=  (1 << (t0 << 1));
    }    	
    
	delay(500);
     
    LCDGPIO->ODR |= (1 << LCD_RES);
	delay(100);
    LCDGPIO->ODR &= ~(1 << LCD_RES);
	delay(100);
    LCDGPIO->ODR |= (1 << LCD_RES);
    ...
    lcd_putstring(0, 60, (char*)"ILI9341 parallel mode demo", 1, WHITE, bcolor);
    lcd_putstring(0, 30, (char*)"for STM32F4", 1, LIGHTGREEN, bcolor);
    lcd_putstring(0, 0, (char*)"by Peter@micromaker.de", 1, YELLOW, bcolor);
		delay(100);
    while(1)
    {   
	}		
