...
int main(void);
 ...
 //  LCD-Display  //
///////////////////
#define LCD_DDR0 DDRC    //Control lines
#define LCD_DDR1 DDRD    //Data
#define LCD_PORT0 PORTC  //Port for LCD commands
#define LCD_PORT1 PORTD  //Port for LCD data
#define RES 0
#define E1  1
#define E2  2
#define RW  3
...
void lcd_write(unsigned char, int, int);
void lcd_init(void);
void lcd_cls(void);
void lcd_gotoxy(int, int, int);
void lcd_putchar(int, int, char, int);

  ///////////////////////////////
 //   L   C   D   Functions   //
///////////////////////////////
void lcd_write(unsigned char value, int lcd_side, int cmd)
{
	switch(lcd_side)
	{
		case 1:	LCD_PORT0 &= ~(1 << E1);
		        break;
        case 2: LCD_PORT0 &= ~(1 << E2);
                break;
        default: LCD_PORT0 &= ~(1 << E1);
                 LCD_PORT0 &= ~(1 << E2);       
    }             
...
void lcd_init(void)
{
    ...
    //Display ON
    lcd_write(0b10101111, 3, 1);
    
    //Display start line
    lcd_write(0b11000000, 3, 1);
    
    //Page address set
    lcd_write(0b10111000, 3, 1);
    
    //Col address set
    lcd_write(0b00000000, 3, 1);
}
...
void lcd_cls(void)
{
	...
	{
	    //Page address set
        lcd_write(0b10111000 + t0, 3, 1);
        
    	//Col address set
        lcd_write(0b00000000 + 0, 3, 1);
        
	    for(t1 = 0; t1 < 79; t1++)
	    {
            lcd_write(0b00000000,  3, 0);
         }   
     }   
}

void lcd_gotoxy(int x, int y, int lcd_side)
{
	//X-pos
	lcd_write(0b00000000 + x, lcd_side, 1); //Col set
	
	//Y-pos
	lcd_write(0b10111000 + y, lcd_side, 1); //Page address
}

void lcd_putchar(int x, int y, char ch, int lcd_side)
{
	int t1;
	
	lcd_gotoxy(x, y, lcd_side);	
	
	for(t1 = 0; t1 < 6; t1++)
	{
	    lcd_write(font[ch - 32][t1], lcd_side, 0);      
	}    
}	

void lcd_putstring(int x, int y, char *s, int lcd_side)
{
	int x1 = x;
	while(*s)
	{
		lcd_putchar(x1, y, *s++, lcd_side);
		x1 += FONTWIDTH;
	}
}		
...
{
	//Set ports for LCD
	LCD_DDR0 = 0x1F; //Cmd-Port
	LCD_DDR1 = 0xFF; //Data-Port
    ...
    //lcd_putchar(0, 0, 65, 1);
    lcd_putstring(0, 0, "Micromaker", 1);
    lcd_putstring(0, 1, "    Peter", 1);
    lcd_putstring(0, 2, "Driver for", 1);
    
    
    lcd_putstring(0, 0, ".de ", 2);
    lcd_putstring(0, 1, "Baier ", 2);
    lcd_putstring(0, 2, "LCD 12232", 2);
    
    lcd_putstring(0, 3, "(C) 2022", 1);
