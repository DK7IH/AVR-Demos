///////////////////////////////////////////////////////////////////
//            Module 7 Using rotary encoder     Version 2        //
///////////////////////////////////////////////////////////////////
...
int main()
{
	...
	PCMSK0 |= (1<<PCINT1);  //Enable 1 encoder pin as interrupt source
	PCICR |= (1<<PCIE0);    //Enable pin change interupts for interrupt bank 0
		
    //Timer 1 as counter for Milliseconds
    TCCR1A = 0;                                          // normal mode, no PWM
    TCCR1B = (1 << CS10) | (1 << CS11) | (1 << WGM12);   //Prescaler = 1/64 based on system clock 8 MHz
                                                         //125000 increments/sec and enable reset of counter register
                                                        
	OCR1AH = (125 >> 8);                                 //Load compare values to registers
    OCR1AL = (125 & 0x00FF);
	TIMSK1 |= (1<<OCIE1A);
    ...
    for(;;) 
	{
		...
		if(ms > 1000) //Reset LEDs after 1s
		{
			ms = 0;
			PORTD |= (1 << 0); //All LEDs off
	        PORTD |= (1 << 1);
	    }
	}
	return 0;
